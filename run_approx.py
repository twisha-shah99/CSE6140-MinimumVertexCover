#!/usr/bin/env python3
"""Run Approx on all test, small, and large instances.

Outputs (.sol) are written to:
  outputs/Approx/test/
  outputs/Approx/small/
  outputs/Approx/large/

Per-instance wall-clock time and relative error are printed.
"""
import os
import subprocess
import sys
...
OUT_ROOT = os.path.join(BASE, "outputs", "Approx")
MVC      = os.path.join(BASE, "mvc.py")

DATASETS = {
    "test":  ([f"test{i}"  for i in range(1, 6)],  10),
    "small": ([f"small{i}" for i in range(1, 19)], 10),
    "large": ([f"large{i}" for i in range(1, 13)], 10),
}
...
def main():
    ...
    for dataset, (names, cutoff) in DATASETS.items():
        ...
        for name in names:
            ...
            sol_file = os.path.join(out_dir, f"{name}_Approx_{cutoff_str}.sol")
            got = None
    ...
    for dataset, name, elapsed, got, ref in results:
        ...
        ref_str = str(ref) if ref is not None else "N/A"
        got_str = str(got) if got is not None else "N/A"
        print(f"{dataset:<8} {name:<12} {elapsed:>8.2f} {got_str:>8} {ref_str:>6} {rel_err:>8}")
